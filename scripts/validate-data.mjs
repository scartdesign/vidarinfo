...
ok(topics.length === 734, 'Očekivano 734 tema, pronađeno ' + topics.length);
...
ok(topTopics('rs virus')[0]?.id==='rsv', 'Upit RS virus mora voditi na RSV');
ok(topTopics('appendicitis')[0]?.id==='apendicitis', 'Upit appendicitis mora voditi na apendicitis');
ok(topTopics('gastroenteritis')[0]?.id==='stomacni-virus', 'Upit gastroenteritis mora voditi na proliv/povraćanje');
ok(topTopics('oralni karcinom')[0]?.id==='rak-usne-duplje', 'Upit oralni karcinom mora voditi na rak usne duplje');
...
ok(topTopics('polip grlica')[0]?.id==='cervikalni-polip', 'Upit polip grlica mora voditi na cervikalni polip');
ok(topTopics('eritema nodosum')[0]?.id==='eritema-nodosum', 'Upit eritema nodosum mora voditi na temu');
ok(topTopics('rebound congestion')[0]?.id==='rinitis-medikamentoza', 'Rebound congestion mora voditi na rinitis medikamentozu');
ok(topTopics('tesko gutam')[0]?.id==='disfagija', 'Upit tesko gutam mora voditi na disfagiju');
ok(topTopics('tecnost oko srca')[0]?.id==='perikardni-izliv', 'Upit tecnost oko srca mora voditi na perikardni izliv');
ok(topTopics('tromboflebitis')[0]?.id==='povrsinski-tromboflebitis', 'Upit tromboflebitis mora voditi na površinski tromboflebitis');
ok(topTopics('limfedem')[0]?.id==='limfedem', 'Upit limfedem mora voditi na limfedem');
ok(topTopics('lipedem')[0]?.id==='lipedem', 'Upit lipedem mora voditi na lipedem');
ok(topTopics('nizak natrijum')[0]?.id==='hiponatremija', 'Upit nizak natrijum mora voditi na hiponatremiju');
ok(topTopics('somnambulizam')[0]?.id==='mesecarenje', 'Somnambulizam mora voditi na mesečarenje');
ok(topTopics('voda oko pluca')[0]?.id==='pleuralni-izliv', 'Upit voda oko pluca mora voditi na pleuralni izliv');
ok(topTopics('apsces dojke')[0]?.id==='apsces-dojke', 'Upit apsces dojke mora voditi na apsces dojke');
ok(topTopics('altitude sickness')[0]?.id==='visinska-bolest', 'Altitude sickness mora voditi na visinsku bolest');
ok(topTopics('osip oko usta')[0]?.id==='perioralni-dermatitis', 'Upit osip oko usta mora voditi na perioralni dermatitis');
...
ok(topTopics('patelarna tendinopatija')[0]?.id==='patelarna-tendinopatija', 'Patelarna tendinopatija mora biti prvi rezultat');
ok(topTopics('it band')[0]?.id==='it-band-sindrom', 'IT band mora voditi na iliotibijalni sindrom');
ok(topTopics('trohantericni bursitis')[0]?.id==='gtps', 'Trohanterični bursitis mora voditi na GTPS');
ok(topTopics('mortonov neurom')[0]?.id==='mortonov-neurom', 'Mortonov neurom mora biti prvi rezultat');
ok(topTopics('metatarzalgija')[0]?.id==='metatarzalgija', 'Metatarzalgija mora biti prvi rezultat');
ok(topTopics('hallux rigidus')[0]?.id==='hallux-rigidus', 'Hallux rigidus mora biti prvi rezultat');
ok(topTopics('krv u oku')[0]?.id==='subkonjunktivalno-krvarenje', 'Krv u oku mora voditi na subkonjunktivalno krvarenje');
ok(topTopics('pterigijum')[0]?.id==='pterigijum', 'Pterigijum mora biti prvi rezultat');
ok(topTopics('glositis')[0]?.id==='glositis', 'Glositis mora biti prvi rezultat');
ok(topTopics('mukokela')[0]?.id==='mukokela', 'Mukokela mora biti prvi rezultat');
ok(topTopics('orhitis')[0]?.id==='orhitis', 'Orhitis mora biti prvi rezultat');
...
ok(topTopics('spusten kapak')[0]?.id==='ptoza-kapka', 'Upit spusten kapak mora voditi na ptozu');
ok(topTopics('hladne noge')[0]?.id==='hladne-sake-stopala', 'Upit hladne noge mora voditi na hladne ekstremitete');
ok(topTopics('zelena stolica')[0]?.id==='zelena-stolica', 'Upit zelena stolica mora voditi na odgovarajuću temu');
ok(topTopics('bol ispod desnog rebra')[0]?.id==='bol-desno-ispod-rebra', 'Upit bol ispod desnog rebra mora voditi na simptomsku temu');
ok(topTopics('bol ispod levog rebra')[0]?.id==='bol-levo-ispod-rebra', 'Upit bol ispod levog rebra mora voditi na simptomsku temu');
...
const urgentRules=[
  ...
  /\b(hemikalij\w*|varikina|izbeljivac|kiselina)\b.{0,18}\b(oko|oku)\b|\b(oko|oku)\b.{0,18}\b(hemikalij\w*|varikina|izbeljivac|kiselina)\b|\b(probodeno oko|probio oko|probola oko)\b/,
...
const urgentNegativeQueries=[
  ...
  'visok pritisak','migrena','erektilna disfunkcija','krvarenje iz nosa','napad panike','ne disem na nos','suncanica','opekotina od sunca','elektricni bol u ruci','krv u ispljuvku','tesko mokrim','nocno mokrenje','sampon u oku'
];
